...
## 3.5 特徵選擇方法原理 (Feature Selection Schemes)

為了優化模型的複雜度並提升泛化能力，我們探討了以下五種特徵選擇方案：

1.  **前向選擇法 (Forward Selection)**：
    屬於**包裹法 (Wrapper Method)**。從空特徵集開始，在每一步疊代中評估所有尚未選入的候選特徵，並將使模型擬合效能（如 $R^2$ 或訓練集分數）提升最顯著的特徵加入集合中，直到達到預設特徵數。
2.  **後向消除法 (Backward Elimination)**：
    同樣是**包裹法**。從包含所有特徵的完整集合開始，在每一步疊代中評估並移除對模型效能影響最小（或移除後模型指標受損最少）的特徵，直到集合縮小到預設的特徵數量。
3.  **遞迴特徵消除法 (Recursive Feature Elimination, RFE)**：
    屬於**包裹法**。利用指定的基底估計器（如線性回歸、隨機森林等），根據模型訓練後產生的係數大小（Coefficients）或特徵重要性權重（Feature Importances），在每一步中剔除權重最小的特徵並重新擬合模型，遞迴進行直到保留預定數量的特徵。
4.  **過濾法 - F-檢定 (SelectKBest - F-regression)**：
    屬於**過濾法 (Filter Method)**。獨立計算每個特徵與目標變數之間的單變量線性相關性（使用 F 檢定），並依據統計顯著性（F 值大小）進行排名，直接挑選 F 值最大的前 K 個特徵。計算速度極快，但不考慮特徵之間的共線性與交互作用。
5.  **過濾法 - 互資訊法 (SelectKBest - Mutual Information)**：
    屬於**過濾法**。利用非參數方法估算特徵與目標之間的熵與互資訊值 (Mutual Information score)，能夠有效捕捉特徵與目標之間的非線性關係。特徵之間的關聯性獨立評估，對於發現複雜模式非常有用，但在處理高度相關特徵時可能產生冗餘。
